...
2.之前连接过的ip地址出现在add new device弹窗的ip下拉地址列表中（用户也可以手动输入）
3.当主题是浅色的时候，statusbar设置为light，当主题是深色，statusbar设置为dark
4.教程弹窗点开后，点击复制，复制结束要toast提示
...
1.详细教程要做成可点击的样式
2 原来mdns是可以自动发现的，现在不行了，看下link项目，mdns是通过udp广播来发现的

1.目前打包成安装包后（win和mac），安装后默认不是开机启动，需要安装后自动设置为开机启动
2.windows用wix安装后，桌面没有快捷方式，也没出现在应用列表


1.windows没有出现在桌面和开机菜单，请检查