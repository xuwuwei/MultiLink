...
echo ""
echo "[5/5] 验证构建结果..."
APP_PATH="target/universal-apple-darwin/release/bundle/macos/MultiLinkServer.app"
DMG_PATH="target/universal-apple-darwin/release/bundle/dmg"

if [ -d "$APP_PATH" ]; then
    echo "    ✓ 应用包: $APP_PATH"
    echo "    ✓ 架构信息:"
    lipo -info "$APP_PATH/Contents/MacOS/MultiLinkServer" | sed 's/^/      /'
else
    echo "    ✗ 应用包未找到"
    exit 1
fi
...
echo ""
echo "    签名信息:"
codesign -dv "$APP_PATH" 2>&1 | grep -E "(Signature|Authority|TeamIdentifier)" | sed 's/^/      /' || echo "      未签名"
...
echo ""
echo "DMG 文件位置:"
echo "  $DMG_PATH/MultiLinkServer_0.1.0_universal.dmg"
echo ""
echo "可以直接分发此 DMG 文件，它同时支持 Intel 和 Apple Silicon Mac"
echo ""
echo "注意: 首次运行时需要在 系统偏好设置 > 安全性与隐私 > 辅助功能 中授权"
echo "      同时需要允许网络连接权限"
